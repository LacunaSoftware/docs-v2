...
// Hide any doc marked `unlisted: true` from the autogenerated sidebars in BOTH
// dev and production. Docusaurus's `unlisted` frontmatter only drops a page from
// the sidebar in production builds; this generator also removes it during
// `docusaurus start`. The pages stay built and reachable by direct URL — they
// are simply not navigable from the menus. Applied to every docs instance that
// autogenerates sidebars (PT, EN, and the pt-br mirror).
async function hideUnlistedSidebarItems(generatorArgs: any) {
  const {defaultSidebarItemsGenerator, ...args} = generatorArgs;
  const items = await defaultSidebarItemsGenerator(args);
  const unlistedIds = new Set(
    (args.docs as Array<{id: string; frontMatter?: {unlisted?: boolean}}>)
  ...
  ? [
      ...
      [
        '@docusaurus/plugin-content-docs',
        {
          ...
          sidebarItemsGenerator: hideUnlistedSidebarItems,
        },
      ],
