...
{/* !! NOTE TO DOCUMENTERS !! When copying the query below over to some other application, refer to the app's source code to check if, like Amplia, it has `RootRoles.Owner = 0`. Some applications actually have `RootRoles.Owner = 1`. In such cases, the correct `RootRoleFlags` value on the query is actually **2** ! */}
