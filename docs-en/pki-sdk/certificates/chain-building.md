...
The library offers the use of a certificate store in memory (`Lacuna.Pki.Stores.MemoryCertificateStore`) and 
...
The parameter can be passed either when the certificate is loaded (methods `Lacuna.Pki.PKCertificate.Decode(System.Byte[],Lacuna.Pki.Stores.ICertificateStore)`
or
`Lacuna.Pki.PKCertificate.Decode(System.String,Lacuna.Pki.Stores.ICertificateStore)`)
how much on the property @Lacuna.Pki.CertificateValidationOptions.CertificateStore validation options (`Lacuna.Pki.CertificateValidationOptions`).
