...
###  *ElectronicSignature* Settings {#electronic-settings}
...
###  *Iugu* Settings (v1.27.0) {#iugu-settings}
...
###  *Whatsapp* Settings (v1.56.0) {#whatsapp-settings}
...
###  *Push Notification* Settings (v1.54.0) {#push-notification}
...
###  *Access Control* Settings (v1.63.0) {#access-control-settings}
...
###  *Email* Settings (v1.66.0) {#app-email-settings}
...
###  *Signature Types* Settings (v1.68.0) {#app-signature-types}
...
###  *Totals* Settings (v1.72.0) {#app-totals-settings}
...
###  *Document Access Validation* Settings (v2.7.0) {#document-access-validation}
...
###  *IdentifierScope* Settings (v2.11.0) {#identifier-scope-settings}
...
Since version 2.18.0, when using multiple identifier types the mode of each type is respected: a type whose mode is not Nullable or NullableUnique is required whenever that type is selected for a user. To require that a user has at least one identifier of any allowed type, use the `RequireAtLeastOneIdentifier` setting.
...
* **IdentifierModes**: comma-separated list of `IdentifierType=Mode` pairs defining the uniqueness behavior for each identifier type. Replaces the deprecated `General.CountryIdentifierMode` setting. Available modes are `Unique`, `NonUnique`, `Nullable` and `NullableUnique`. Example: `Cpf=Nullable,EcuadorBceId=NullableUnique`. **If `EnableMultipleIdentifiersPerUser` is `true`, this setting must be defined for all identifier types included in `AllowedIdentifierTypes`.** Since version 2.18.0, types with a non-nullable mode are allowed when multiple identifiers are enabled: the identifier value becomes required whenever that type is selected.
* **RequireAtLeastOneIdentifier** (default: `false`, v2.18.0): if `true`, every user must have at least one identifier of an allowed type, even when all identifier modes are nullable. Identifiers of types that are no longer listed in `AllowedIdentifierTypes` do not satisfy this rule. Can only be used when `EnableMultipleIdentifiersPerUser` is `true` — otherwise the application fails to start.
